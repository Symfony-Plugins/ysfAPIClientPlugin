...
function _call_component($moduleName, $componentName, $vars)
{
  ...
  {
    $apiComponentData = ysfAPIClient::getInstance()->getAllComponentData($moduleName, $componentName);
    if(is_array($apiComponentData))
    {
      // add api data to component
      $componentInstance->getVarHolder()->add($apiComponentData);
    }
  }
